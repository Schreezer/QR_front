...
export default function Home() {
  ...
  const startAutomationMutation = useMutation({
    mutationFn: (url: string) => 
      apiRequest("POST", "/api/automation/start", { 
        url, 
        value1: settings?.value1 || "14", 
        value2: settings?.value2 || "15"
      }),
    onSuccess: async () => {
      toast({
  ...
  const { data: statusData } = useQuery({
    ...
    onSuccess: (data) => {
      if (data) {
        ...
        if (data.instances) {
          setBrowserInstances(data.instances);
        }
  ...
  const handleStartAutomation = () => {
    ...
    setAutomation({
      ...
      steps: [
        ...
        },
        {
          id: "fill",
          title: "Filling forms",
          description: `Finding text fields and entering values (${settings?.value1 || "14"} and ${settings?.value2 || "15"})`,
          status: "pending",
        },
        {
          id: "submit",
    ...
    setBrowserInstances([
      { id: 1, value: settings?.value1 || "14", status: "loading" },
      { id: 2, value: settings?.value2 || "15", status: "loading" },
    ]);
    ...
    <div className="bg-slate-100 min-h-screen font-sans">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <header className="mb-6">
          <h1 className="text-2xl font-semibold text-slate-800">QR Form Automation</h1>
          <p className="text-slate-600 mt-1">Scan a QR code to open and fill forms automatically</p>
        </header>
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          ...
          <div className="flex border-b border-slate-200">
            <button 
              className={`px-4 py-3 font-medium ${activeTab === "scan" ? "text-primary border-b-2 border-primary" : "text-slate-600"}`}
              onClick={() => setActiveTab("scan")}
            >
              Scan
            </button>
            <button 
              className={`px-4 py-3 font-medium ${activeTab === "settings" ? "text-primary border-b-2 border-primary" : "text-slate-600"}`}
              onClick={() => setActiveTab("settings")}
            >
              Settings
            </button>
            <button 
              className={`px-4 py-3 font-medium ${activeTab === "history" ? "text-primary border-b-2 border-primary" : "text-slate-600"}`}
